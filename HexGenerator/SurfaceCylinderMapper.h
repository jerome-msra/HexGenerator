...
namespace MeshLib
{
	...
	public:
		...
		MAP_CYLINDER & _cylinderOption(){ return m_cylinderOption; };

	protected:

		void __mapDisk();

		void __mapCylinder_u();
		void __mapCylinder_v();
	...
	template<typename M, typename T>
	void CSurfaceCylinderMapper<M, T>::__mapCylinder_u()
	{
		...
		for (std::vector<M::CLoop*>::iterator lIter = domainLoops.begin(); lIter != domainLoops.end(); lIter++)
		{
			...
			for (std::list<M::CHalfEdge*>::iterator hIter = halfedges.begin(); hIter != halfedges.end(); hIter++)
			{
				...
				//if (pVS->group() != 0 || pVT->group() != 0)
				if (pVS->group() != 0 && pVT->group() != 0)
				{
					int g = (pVS->group() != 0) ? pVS->group() : pVT->group();
		...
		for (M::MeshVertexIterator vIter(m_pMesh); !vIter.end(); vIter++)
		{
			...
			M::CVertex * domainVert = m_pDomain->findVertex(pV->point(), pV->group());
			...
			for (int g = 1; g < 3; g++)
			{
				...
				for (std::vector<M::CHalfEdge*>::iterator hIter = halfedges.begin(); hIter != halfedges.end(); hIter++)
				{
					...
					{
						M::CVertex * originV0 = m_pMesh->findVertex(pV0->point(), g);
						M::CVertex * originV1 = m_pMesh->findVertex(pV1->point(), g);
						found = true;
				...
				if (!found)
				{
					...
					for (std::vector<M::CHalfEdge*>::iterator hIter = halfedges.begin(); hIter != halfedges.end(); hIter++)
					{
						...
						CPoint2 uv = pTV->uv();
						if (m_pMesh->findVertex(pTV->point(), g) != nullptr)
						{
							uvMap[uv[0]] = pTV;
						}
					}
					...
					M::CVertex * originVert = m_pMesh->findVertex(vert->point(), g);
	...
	template<typename M, typename T>
	void CSurfaceCylinderMapper<M, T>::__mapCylinder_v()
	{
		...
		for (M::MeshVertexIterator vIter(m_pMesh); !vIter.end(); vIter++)
		{
			...
			for (int g = 1; g < 3; g++)
			{
				...
				for (std::vector<M::CHalfEdge*>::iterator hIter = halfedges.begin(); hIter != halfedges.end(); hIter++)
				{
					...
					if ((uv0[1] >= uv[1] && uv1[1] <= uv[1]) || (uv0[1] <= uv[1] && uv1[1] >= uv[1]))
					{
						M::CVertex * originV0 = m_pMesh->findVertex(pV0->point(), g);
						M::CVertex * originV1 = m_pMesh->findVertex(pV1->point(), g);
						found = true;
						if (originV0 != nullptr && originV1 != nullptr)
						{
							...
							double lambda = (uv[1] - uv0[1]) / (uv1[1] - uv0[1]);
							CPoint2  huv = huv0 + (huv1 - huv0) * lambda;

							huvCandidates[originV0->group() - 1] = huv;
							domainDists[originV0->group() - 1] = fabs(uv[0] - (uv0[0] + uv1[0]) / 2.0);
						}
						else if (originV0 != nullptr)
						{
							CPoint2 huv0 = originV0->huv();
						...
						else if (originV1 != nullptr)
						{
							CPoint2 huv1 = originV1->huv();
							huvCandidates[originV1->group() - 1] = huv1;
							domainDists[originV1->group() - 1] = fabs(uv[0] - (uv0[0] + uv1[0]) / 2.0);
						}
				...
				if (!found)
				{
					...
					M::CVertex * vert = (fabs(uv[1] - firstVert->uv()[1]) < fabs(uv[1] - endVert->uv()[1])) ? firstVert : endVert;
					M::CVertex * originVert = m_pMesh->findVertex(vert->point(), g);
					huvCandidates[originVert->group() - 1] = originVert->huv();
					domainDists[originVert->group() - 1] = fabs(uv[0] - vert->uv()[0]);
				}
			}
	...
	void CSurfaceCylinderMapper<M, T>::__mergeDiskCylinder()
	{
		...
		for (size_t i = 0; i < m_diskMeshes.size(); i++)
		{
			...
			{
				M::CFace * pF = *fIter;
				M::CFace * testFace = m_cylinderMesh->idFace(pF->id());
				if (testFace != nullptr)
				{
					continue;
				}
	...
	void CSurfaceCylinderMapper<M, T>::_map(M * domain, double height)
	{
		...
		m_pDomain = domain;
		m_pDomain->generatePointMap();

		std::cout << "[map_surface]Mapping Cylinder..." << std::endl;
		switch (m_cylinderOption)
		{
		case MAP_CYLINDER::MAP_U:
			__mapCylinder_u();
			break;
		case MAP_CYLINDER::MAP_V:
			__mapCylinder_v();
			break;
		default:
			break;
